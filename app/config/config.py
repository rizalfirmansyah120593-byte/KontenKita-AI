...
def load_config():
    ...
    config = _load_toml_config(config_file)
    _load_dotenv_file(env_file)

    # Environment variables are useful for local deployments and take
    # precedence only when a corresponding value is present in .env.
    app_config = config.setdefault("app", {})
    env_mapping = {
        "MPT_LLM_PROVIDER": "llm_provider",
    ...
    for environment_name, config_name in env_mapping.items():
        value = os.getenv(environment_name, "").strip()
        if not value:
            continue
        if config_name.endswith("_api_key") and config_name in {
            "pexels_api_key",
            "pixabay_api_key",
        }:
            app_config[config_name.replace("_api_key", "_api_keys")] = [value]
        else:
            app_config[config_name] = value

    return config


def _load_dotenv_file(path: str):
    """Load simple KEY=VALUE pairs without adding a dotenv dependency."""
    if not os.path.isfile(path):
        return
    try:
        with open(path, encoding="utf-8") as env_handle:
        ...
        logger.warning(f"cannot load .env file: {exc}")
...
project_name = _cfg.get("project_name", "KontenKita AI")
